...
import SAKPC.Models.Message
import com.sksamuel.avro4s.{AvroInputStream, AvroNamespace, AvroNamespaceable, AvroOutputStream, AvroSchema, RecordFormat}
...
object SAKPC extends App {
  ...
  object Models1 {
    ...
    case class UserMessage(name: String) extends Message
    @AvroNamespace("")
    case class SystemMessage(name: String, newField: Option[String] = None, supField: Option[String] = None, newField1: Option[String] = Some("SuperRanga")) extends Message
    val msgAvroSchema = AvroSchema[Message]
  ...
  println(Models.msgAvroSchema)
  println(())
  println(Models1.msgAvroSchema)
  ...
  val f = for(i <- 1 to 100) yield {
    ...
    val data = if(scala.util.Random.nextInt(100) % 2 == 0) Models.UserMessage(s"Hello ${key}") else Models.SystemMessage(s"Hello System Message ${key}", None, Some("Super"))
    val b = new ByteArrayOutputStream()
    val k = AvroOutputStream.json[Models.Message].to(b).build(Models.msgAvroSchema)
    k.write(data)
    k.flush()
  ...
  val consumeFut = consumer.scheduledPoll(10000, (it) => {
    Try {
      it.foreach { e => {
        println(s"key: ${e.key()}")
        println(s"valueInValue ${new String(e.value())}")
        println(s"value: ${Try(AvroInputStream.json[Models1.Message].from(e.value()).build(Models1.msgAvroSchema).tryIterator.toList)}")
      }
      }
    }
  }, (ex) => {
